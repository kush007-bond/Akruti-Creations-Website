      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
          ...
          >
            ...
            <p className="text-lg text-surface/60 font-medium">
              We bridge the gap between abstract design and physical reality through a rigorous four-stage workflow.
            </p>
          </motion.div>
          
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-px bg-surface/10 border border-surface/10 rounded-3xl overflow-hidden">
            {processSteps.map((item, idx) => (
              ...
              >
                <div className="flex justify-between items-start mb-10">
                  <div className="w-14 h-14 rounded-2xl bg-base flex items-center justify-center text-accent group-hover:scale-110 transition-transform">
                    {item.icon}
                  </div>
                  <span className="text-5xl font-black text-surface/5 group-hover:text-accent/20 transition-colors">{item.step}</span>
                </div>
                <h4 className="text-2xl font-bold mb-4 tracking-tight">{item.title}</h4>
                <p className="text-surface/60 leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
